...
if __name__ == '__main__':
    ...
    try:
        ...
        controller = Controller(tello, joy)
        x = threading.Thread(target=controller.run, daemon=True)
        x.start()
    except:
    ...
    while True:
        ...
        if key == ord("t"):
            ...
            s = test2.starting(tello)
            s.run()
            tello_height.goto_height(tello, 30)
        elif key == ord("q"):
            f.close()
            f1.close()
            break
    ...
    parser1()
    sleep(5)
    final_csv_final.getout(ID, "distribution.csv")
